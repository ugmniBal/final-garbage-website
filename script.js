...
document.getElementById('locationInput').addEventListener('keydown', function(e) {
  if (e.key === 'Enter') {
    e.preventDefault(); // 자동완성 선택 안했으면 Enter 막기
  }
});

function autocomplete() {
  const input = document.getElementById("locationInput").value.trim().replace(/\s+/g, '');
  suggestionsEl.innerHTML = "";
  if (!input) return;
  currentList = data.filter(d => {
    const full = \`\${d.시도명}\${d.시군구명}\${d.관리구역명}\`.replace(/\s+/g, '');
...
function searchInfo() {
  ...
  const match = currentList.find(d => {
    const full = \`\${d.시도명}\${d.시군구명}\${d.관리구역명}\`.replace(/\s+/g, '');
    return full === input;
  });
